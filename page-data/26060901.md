...
使用差分得到现有的覆盖范围可以覆盖哪些格子之后，我们找出没有被覆盖的格子的横、纵坐标的最小值 $x_{\min}, y_{\min}$ 和最大值 $x_{\max}, y_{\max}$。显然，如果 $x_{\max} - x_{\min} > 2k$ 或 $y_{\max} - y_{\min} > 2k$，那么无论如何，也找不到一个如上所述的正方形，与已有的覆盖范围一起，将整个网格覆盖。否则，如果每个未被覆盖的网格坐标 $(x, y)$ 均满足
